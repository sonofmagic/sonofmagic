...
export async function runCli(runOptions: RunCliOptions = {}) {
  ...
  cli.command('summary', 'Show a short intro and exit')
  cli.command('links', 'Show all public links and exit')
  cli.command('contact', 'Show contact links and exit')
  cli.command('export', 'Show the profile as Markdown and exit')
  cli.command('health', 'Check the local profile data and exit')
  ...
  cli.example(bin => `$ ${bin} health`)
  cli.example(bin => `$ ${bin} projects`)
